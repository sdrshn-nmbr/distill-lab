...
def test_concurrent_publication_is_idempotent_and_verified(tmp_path: Path) -> None:
    ...
    assert len(list((tmp_path / "private" / "objects").rglob("*.blob"))) == 1
...
def test_public_reference_cannot_read_private_bytes(tmp_path: Path) -> None:
    store = LocalArtifactStore(tmp_path)
    private = store.put_bytes(
        b"hidden", media_type="application/octet-stream", sensitivity="private"
    )
    public = private.model_copy(update={"sensitivity": "public"})
